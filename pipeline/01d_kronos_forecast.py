...
_MULT_TABLE = [
    ...
    (1.5, 1.12, 0.88),
    (0.5, 1.06, 0.94),
    (0.0, 1.00, 1.00),  # neutral / very small move
]


def _kronos_mult(spread_type: str, forecast_pct: float) -> float:
    """Map a directional forecast to a score multiplier for the given spread type."""
...
def _write_neutral_signals(tickers: list, installed: bool = False) -> None:
    """Write ×1.00 signals for every ticker so the pipeline can continue."""
    signals = {
        t: {
            "forecast_pct":           0.0,
            "direction":              "neutral",
...
def main():
    ...
    # ── Kronos not installed — graceful degradation ────────────
    ...
    for ticker in tickers:
        ...
        try:
            ...
            # Kronos expects df without the index — pass plain OHLCV DataFrame
